...
// and the literal commands that would do it. Folders not present in DB are
// treated as Unknown and never offered for cleaning until someone adds an
// entry for them.
package knowledge
...
// Lookup returns the dictionary entry for name by itself, with no context
// about sibling folders. Exact folder names are checked first, then a
// small set of patterns for folders whose name varies (typically by
// embedding a version number). Names matching neither return a zero-value
// Unknown entry. Most callers that have a folder listing available should
// prefer Effective, which additionally accounts for sibling versions.
...
// Effective is Lookup adjusted for context: if name is a JetBrains
// per-version cache folder and siblings (the other folder names in the
// same directory) contains a strictly newer version of the same IDE, the
// older version's cache is fully superseded — nothing will ever use it
// again — so it's promoted to Safe with a whole-folder delete, instead of
// the conservative subpath-only cleanup Lookup gives the current version.
func Effective(name string, siblings []string) Entry {
	base := Lookup(name)
		...
		if y2 > year || (y2 == year && m2 > minor) {
			return Entry{
				...
				Effects: "A newer version of " + product + " is installed, so this version will never run again " +
					...
					"caches, everything) is safe to delete outright. If you do reinstall this exact version, it " +
					"just reindexes your projects and re-downloads plugins like a fresh install would.",
...
// jetbrainsVersionRe matches a JetBrains IDE's per-version cache folder,
// e.g. "GoLand2026.1", "IntelliJIdea2024.3", "WebStorm2026.2", capturing
// the product name, year, and minor version separately for comparison.
...
// parseJetBrainsVersion splits a per-version JetBrains cache folder name
// into product + comparable (year, minor) version numbers.
...
var patterns = []patternEntry{
	{
		...
		build: func(name string) Entry {
			commands := make([]string, 0, len(jetbrainsCleanPaths)+1)
...
var db = map[string]Entry{
	...
	"dev.kdrag0n.MacVirt": {
		Score: Risky,
		Description: "Cache for MacVirt, a lightweight VM/container runtime (used by tools like OrbStack-style " +
			"apps) for running Linux VMs on macOS. Depending on version this can include VM disk overlay data, " +
			"downloaded base images, or just UI/update scratch files — the exact contents vary by version.",
